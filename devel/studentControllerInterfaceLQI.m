classdef studentControllerInterfaceLQI < matlab.System
    properties (Access = private)
        ...
        tau = 0.025;
        u_prev = 0;
        Sigma_vv = diag([0.01,0.01,0.01,0.01]);
        Sigma_ww = 0.01*eye(2);
        p_integral = 0;
    end
    methods(Access = protected)
        ...
        %   V_servo: voltage to the servo input. 
            ...
            p_integral = p_integral + (x_hat(1) - p_ball_ref)*dt;
            theta_ref = asin(a_ball_ref*len/(r_g*g));

            A_lqr = [1, dt, 0, 0, 0;
                     5/7*(r_g/len)^2*x_hat(4)^2*(cos(x_hat(3)))^2*dt, 1, (5*g/7*r_g/len*cos(x_hat(3)) + 10/7*(len/2-x_hat(1))*(r_g/len)^2*x_hat(4)^2*cos(x_hat(3))*sin(x_hat(3)))*dt, -10/7*(len/2-x_hat(1))*(r_g/len)^2*x_hat(4)*(cos(x_hat(3)))^2*dt, 0;
                     0, 0, 1, dt, 0;
                     0, 0, 0, 1-dt/tau, 0;
                     dt, 0, 0, 0, 1];
            B_lqr = [0;0;0;K_motor/tau*dt;0];
            Q = diag([min(1000,1 + t*999), min(1000,1 + t*999), 1, 1, 1]); % for MATLAB sim
            ...
            x_tilde = [x_hat; p_integral] - [p_ball_ref; v_ball_ref; theta_ref; 0; 0];
